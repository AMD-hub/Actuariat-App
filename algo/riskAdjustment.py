...
def execute(triangle_cumule, triangle_charge, taux, alpha):
    ...
    # Ajustement des paramètres de la loi normale
    mu, sigma = norm.fit(Best_Estimate)

    # Calcul de la VaR au niveau de confiance de 80%
    VAR_alpha = norm.ppf(alpha, mu, sigma)
    # Calcul de l'espérance des meilleures estimations
    Espérance = np.mean(Best_Estimate)

    # Calcul du Risk Adjustment
    RA = VAR_alpha - Espérance

    return Best_estimate_boot, Best_Estimate, mean_best_estimate, RA
